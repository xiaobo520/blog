	...
	<div class="jy_box">
			...
			<div class="zhengwen_l_2">
				<div class="art_box">
					...
					<div class="content_art">
						...
						<div class="zhuanzai">转载请注明：<a href="https://<?=$var_domain?>/jingyan/<?=$cid?>.html" ><?=$title?> | 码代码-大胆哥博客</a></div>
					</div>
				</div>
				...
				<?
				$sql_search="select id,title from lei_jingyan where pass='1' and id<>'".$cid."' order by rand() limit 4 ";
				...
				?>
				<div class="content_guanlian"><i class="icon-chevron-right"></i> <a href="/jingyan/<?=$rs["id"]?>.html" target="_blank" title="<?=$rs["title"]?>"><?=$rs["title"]?></a></div>
				<? }
				?>
		...
		?>
			...
			<div class="zhengwen_l_3_1">
				...
				<div id="huifubox">
					<?
					$sql_1="select count(*) from lei_jingyan_hf where cid='".$cid."' and cid_hf='' and pass='1' ";
					...
					$sql_2="select * from lei_jingyan_hf where cid='".$cid."' and cid_hf='' and pass='1' order by id limit ".$pagestart_1.",".$pagenum_1." ";
					$iloi=0;
						...
						<div id="pinglun_big_box">
							...
							<div class="pinglun_usertitle">
								...
								<div class="hf_message">
									...
									<!--回复中是否有被评论的 二级评论-->
									<?
									$sql_4="select count(*) from lei_jingyan_hf where cid_hf='".$rs_2["id"]."' and pass='1' ";
									...
									?>
										...
										<?
										$sql_3="select * from lei_jingyan_hf where cid_hf='".$rs_2["id"]."' and pass='1' order by id limit 0,5";
										$iloi3=0;
...
function share_art(webid){
	/*var url="https://<?=$var_domain?>/jingyan/<?=$cid?>.html";
	var title="<?=$title?>";
	var pic='<?="https://".$var_domain.str_replace('../','/',$simgpaths)?>';
