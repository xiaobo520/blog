...
if ($action=="edit"){
	...
	if ($_FILES["file"]["error"] !=4){
		...
		$sql="select simgpaths from lei_news where id='".$id."' limit 1 ";
	...
	$sql="update lei_news set title='".$title."',title_code='".$title_code."',indate='".$indate."',laiyuan='".$laiyuan."',hits='".$hits."',pass='".$pass."',message='".$message."',zhiding='".$zhiding."',pinglun='".$pinglun."',guanjianci='".$guanjianci."' where id='".$id."' ";
	$mysqli->query($sql);
...
$sql="select * from lei_news where id='".$id."' limit 1 ";
$result=$mysqli->query($sql);
if($rs=$result->fetch_assoc()){
	$title=$rs["title"];
	$indate=$rs["indate"];
		...
		<table width="100%" >
		  ...
		  <tr>
		   ...
		   <tr>
			...
			<tr>
			  <th>缩略图片</th>
			  <td><input name="file" type="file" id="file" class="input length_5"/>&nbsp; 180*130 不修改，勿上传 &nbsp;&nbsp;&nbsp;<a href="<?=$simgpaths?>" target="_blank" onMouseMove="show(event,'<?=$simgpaths?>','111')" onmouseout="hide(this)" >图片预览</a>&nbsp;&nbsp;&nbsp;<a href="/news/?&see=true&search_open=ok" target="_blank" >列表预览</a>&nbsp;&nbsp;&nbsp;<a href="/news/<?=$id?>.html?see=true" target="_blank" >文章预览</a>
		   ...
		   <tr>
			  ...
			  <td>
				...
				<input type="radio" name="zhiding" value="1" <? if ($zhiding=='1'){?>checked="checked"<? }?>/> 置顶
			  </td>
		  </tr>
		  <tr>
			  <th>是否评论</th>
