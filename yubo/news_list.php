...
if ($action=="del"){
	...
	$sql="select id,simgpaths from lei_news where id='".$id."' and pass<>'1' limit 1";
	$result=$mysqli->query($sql);
	if($rs=$result->fetch_assoc()){

		$sql="delete from lei_news where id='".$id."' and pass<>'1' limit 1";
		$mysqli->query($sql);
...
if ($action=="shangbao" ){//提交
	...
	$sql="select id,pass from lei_news where id='".$id."' limit 1 ";
	$result=$mysqli->query($sql);
	if($rs=$result->fetch_assoc()){
		$pass=$rs["pass"];
	}
	...
	$sql="update lei_news set pass='".$zhuangtai_value."' where id='".$id."'";
	$mysqli->query($sql);
...
<div class="wrap">	
	...
	<div class="h_a">信息检索</div>
		...
		<div class="ul_wrap">
			<ul class="cc">
				<? include_once("news_form_all.php");?>
			</ul>
		</div>
		<div class="btn_side">
			<button class="btn btn_submit loading_it" type="submit">搜索</button>
			&nbsp;&nbsp;
			<button class="btn btn_error loading_it" type="button" onclick="document.location.href='news_add.php?<?=encrypt_url("&page=".$page."&".$pageurl."&time=".time(),$key_url_md_5)?>'">添加</button>
		</div>
		</form>
	</div>
...
<?php
	...
	include_once("news_sql_all.php"); 
	
	$sql_page="select count(*) from lei_news where ".$searchstr."";
	...
	$sql_search="select id,title,laiyuan,indate,hits,zhiding,pass".$search_score." from lei_news where ".$searchstr." ".$px_str." limit ".$pagestart.",".$pagenum." ";
...
?>
			<tr>
			  <td><?=$i?><? if ($aid==$rs["id"]){?> <i class="icon-eye-open"></i><? }?></td>
				...
				<td><?=keywordlight($title_search,clear_all($rs["title"]),$jingdu_search);?><? if ($rs["zhiding"]=='1'){echo '<span style=color:red > [置顶]</span>';}?></td>
				<td><?=$rs["laiyuan"]?></td>
				<td><?=$rs["hits"]?></td>
			    <td><?
				...
				?></td>
			    <td>
					  <a class="loading_it" href="news_edit.php?<?=encrypt_url("&id=".$rs["id"]."&page=".$page."&".$pageurl."&time=".time(),$key_url_md_5)?>"><i class="icon-edit"></i> 详细</a>
					  <?
					   if ( $rs["pass"]!='1'){?>
					  &nbsp;|&nbsp;
