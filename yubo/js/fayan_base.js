function ribao_save(){ 
	...
	var ajaxform=$.post("fayan_save_m.php?sid="+Math.random(),{action:'save',id:id,table:table,message1:message1},function(result){	
...
function ribao_read(id,table){ 
	$("#hide_read").val(id);
	$("#hide_table").val(table);
	$("#readzt").html("读取中，请稍后...");
	$("#readzt").fadeIn(1);
	
	var ajaxform=$.post("fayan_save_m.php?sid="+Math.random(),{action:'read',id:id,table:table},function(result){	
		var result_str=result.split("|");	

		if (result_str[0]=='yes'){
			//UE.getEditor('message1').execCommand('source');
			if (UE.getEditor('message1').queryCommandState('source')==1){ //如果当前是源码编辑模式，返回1，否则返回0
				UE.getEditor('message1').execCommand('source');
				UE.getEditor('message1').focus();
			}
